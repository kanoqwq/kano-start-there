...
<div class="container mx-auto" @click.self="containerClick" ref="container">
  <div class="top-banner flex justify-end items-center">
    ...
    <svg class="kano-icon dark:dark-icon w-8 icon-hover" aria-hidden="false" @click.stop="settingHandler">
      <use xlink:href="#icon-settings" class="rotate"></use>
    </svg>
  </div>
  <div ref="searchEle" class="search relative flex flex-col">
    <div class="search-engine flex overflow-hidden dark:dark-search-engine" ref="searchEngineElement">
      <div class="flex items-center" id="engine_switch_btn" @click.stop="switchEngine">
        <i :class="`icon kano-icon dark:dark-kano-icon iconfont dark:dark-icon ${selectedEngine.icon}`"></i>
      </div>
      <input ref="searchBox" id="searchBox" type="text" autocomplete="off" v-model="searchContent"
        ...
        @input.stop="searchSuggestion(selectedEngine.method)" @keydown.stop="moveSuggestion"
        @mouseenter.stop="eventMouse" @focusin.stop="showHideSearchHistory" @focusout.stop="showHideSearchHistory"
        @mouseleave.stop="eventMouse" class="input pl-3 box-border outline-none dark:input-dark"
        :placeholder="`在${selectedEngine.name}上搜索`" />
      <div class="clear-input" @click.stop="clearContent" v-show="searchContent">
    ...
    <Transition name="suggestion">
      <div @mouseenter.stop="suggestActiveControl" @mouseleave.stop="suggestActiveControl"
        class="search-suggestion top-border absolute dark:dark-suggest-bg" v-if="
          (suggestWords.length && suggestIsShow) || isSearchEngineListMode
        ">
        <ul ref="scrollUl">
          <transition-group name="suggestion-ul">
            <li v-if="!isSearchEngineListMode" class="inner dark:dark-text" :class="{ active: item.isSelected }"
              @click.stop="startSearch(item.title)" v-for="(item, index) in suggestWords" :key="index">
              <span class="searchkey" :id="`key_${index}_${Math.random()}`">{{
                item.title
              }}</span>
              <i v-show="item.allowDel" @click.stop.prevent="delHistory(index)"
 ...
 * @Last Modified time: 2024-11-28 08:57:03
  ...
  import type { SearchEngine, SuggestWords } from '@/types/global';
  ...
  const engineId: number = searchEnginesStore.selectedEngine;
  const selectedEngine = reactive<SearchEngine>({
    ...(searchEnginesStore.searchEngines.find((i) => i.id === engineId) ||
      searchEnginesStore.searchEngines[0]),
  });
  const suggestionIndex = ref(-1);
  const suggestWords = ref<Array<SuggestWords>>([]);
  ...
  const startSearch = (
    ...
    const reqUrl = `${selectedEngine.url + encodeURIComponent(keyWord)} ${filterWords.value}`;
    ...
    async (
      ...
      try {
        ...
        if (searchContent.value && method) {
          const res = await suggestAPI[method](searchContent.value);
